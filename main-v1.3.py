...
class GitHubManagerApp:
    ...
    def _get_initial_local_path(self):
        ...
        try:
            ...
            desktop_path = quick_paths.get("Desktop") # Sửa key để phù hợp với get_quick_access_paths
                ...
                if finished:
                    ...
                    if refresh_target == 'github':
                        ...
                        if action in ['delete_repo', 'rename_repo']:
                            ...
                            self.populate_github_tree()
                        # For actions like 'delete', 'rename_file', 'upload', 'batch_delete'
                        elif current_repo and current_repo == refresh_repo:
                             action_parent_dir_of_item = os.path.dirname(refresh_path).replace("\\", "/") if refresh_path else "" # path of the item's parent
                             
                             # refresh_path from worker:
                             # - single delete: parent dir of deleted item
                             # - batch delete: current_github_context path (where deletion happened)
                             # - rename_file: parent dir of renamed file
                             # - upload: base github_path for upload

                             norm_current_path = current_path.strip('/')
                             norm_refresh_path = refresh_path.strip('/') if refresh_path else "" # This is the path provided by the worker
                             
                             # If the worker indicated the refresh path *is* our current path, refresh.
                             # This covers batch delete, and uploads/renames happening in the current view.
                             if norm_refresh_path == norm_current_path:
                                 print(f"Refreshing current GitHub view ({current_repo}/{current_path}) for action '{action}' affecting current path.")
                                 self.refresh_github_tree_current_view()
                             # If the worker indicated the refresh path is the parent of an item,
                             # and that parent is our current path, refresh.
                             # This covers single item deletions where refresh_path is the item's parent.
                             elif os.path.dirname(norm_refresh_path).strip('/') == norm_current_path and action == 'delete': # More specific for old single delete if it were active
                                 print(f"Refreshing current GitHub view ({current_repo}/{current_path}) for action '{action}' affecting item in current path.")
                                 self.refresh_github_tree_current_view()
    ...
    def delete_selected_github_items_prompt(self, event=None):
        selected_items_info = self.get_selected_github_items_info()
        items_to_delete = [item for item in selected_items_info if item.get('type') != 'repo']

        if not items_to_delete:
            self.log_status("Vui lòng chọn file/thư mục GitHub để xóa.", "WARNING")
            return

        repo_display_name = items_to_delete[0]['repo'] if items_to_delete else self.current_github_context.get('repo', "current")

        files = [item for item in items_to_delete if item['type'] == 'file']
        dirs = [item for item in items_to_delete if item['type'] == 'dir']
        
        confirm_msg = f"Xóa {len(items_to_delete)} mục khỏi Repo '{repo_display_name}':\n"
        if files:
            confirm_msg += "\nFILES:\n" + "\n".join([f" - '{item['name']}'" for item in files[:5]])
            if len(files) > 5: confirm_msg += "\n - ..."
        if dirs:
            confirm_msg += "\n\nTHƯ MỤC:\n" + "\n".join([f" - '{item['name']}'" for item in dirs[:5]])
            if len(dirs) > 5: confirm_msg += "\n - ..."
            confirm_msg += "\n(Lưu ý: Chỉ xóa được thư mục rỗng qua API này)"
        ...
        if messagebox.askyesno("Xác nhận Xóa GitHub", confirm_msg, icon='warning', parent=self.root):
            ...
            self.start_batch_delete_github_items_thread(items_to_delete) # Call the new batch delete method
        else:
            self.log_status("Hủy thao tác xóa.", "INFO")
    ...
    # ... (Giữ nguyên các hàm start/worker cho Upload, Download, Rename) ...
        ...
        while not upload_queue.empty():
            ...
            if os.path.isfile(local_item_path):
                ...
                try:
                    ...
                    success_count += 1 # Count directory itself as processed successfully if listed
                except OSError as e:
    ...
    # --- START: New Batch Delete GitHub Items Threading ---
    def start_batch_delete_github_items_thread(self, items_to_delete):
        if not self.github_handler or not self.github_handler.is_authenticated():
            messagebox.showerror("Lỗi", "Chưa kết nối GitHub.", parent=self.root)
            return

        self.task_id_counter += 1
        task_id = self.task_id_counter
        
        refresh_repo_context = self.current_github_context.get('repo')
        refresh_path_context = self.current_github_context.get('path', "")

        thread = threading.Thread(
            target=self._batch_delete_github_items_worker,
            args=(task_id, items_to_delete, refresh_repo_context, refresh_path_context),
            daemon=True
        )
        
        self.upload_tasks[task_id] = {
            'thread': thread,
            'status': 'Bắt đầu batch delete...',
            'progress': 0,
            'type': 'batch_delete_gh',
            'repo': refresh_repo_context,
            'path': refresh_path_context
        }
        
        item_names_preview = ", ".join([item['name'] for item in items_to_delete[:3]])
        if len(items_to_delete) > 3:
            item_names_preview += "..."
            
        update_queue.put((
            task_id,
            f"Bắt đầu xóa batch ({len(items_to_delete)} mục: {item_names_preview})...",
            0,
            False,
            None 
        ))
    ...
    def _batch_delete_github_items_worker(self, task_id, items_to_delete, refresh_repo_context, refresh_path_context):
        if not self.github_handler or not self.github_handler.is_authenticated():
            update_queue.put((task_id, "Lỗi: Worker không thể xóa vì chưa xác thực GitHub.", 100, True, None))
            return

        num_total_items = len(items_to_delete)
        num_successful = 0
        num_failed = 0
        error_messages = []

        for i, item_info in enumerate(items_to_delete):
            item_name = item_info.get('name', 'Unknown item')
            repo_name_op = item_info.get('repo') # Repo of the item being operated on
            item_path = item_info.get('path')
            item_sha = item_info.get('sha')

            current_progress = int(((i + 1) / num_total_items) * 95) if num_total_items > 0 else 0

            if not all([repo_name_op, item_path, item_sha]):
                msg = f"LỖI: Thiếu thông tin để xóa item '{item_name}' (repo, path, hoặc sha)."
                update_queue.put((task_id, msg, current_progress, False, None))
                num_failed += 1
            ...
            update_queue.put((
                task_id,
                f"Đang xóa ({i+1}/{num_total_items}): {item_name} từ {repo_name_op}...",
                current_progress,
                False,
                None 
            ))

            success, result_message = self.github_handler.delete_item(repo_name_op, item_path, item_sha)

            if success:
                num_successful += 1
                # Don't include full result_message if it's too long or generic
                brief_success_msg = f"Đã xóa: {item_name}." 
                update_queue.put((task_id, brief_success_msg , current_progress, False, None))
            else:
                num_failed += 1
                error_messages.append(f"Lỗi xóa '{item_name}': {result_message}")
                update_queue.put((task_id, f"LỖI xóa {item_name}: {result_message}", current_progress, False, None))
            
            # time.sleep(0.1) # Optional delay

        final_message = f"Hoàn thành batch xóa: {num_successful} thành công"
        if num_failed > 0:
            final_message += f", {num_failed} thất bại."
            # Log detailed errors to console for debugging
            print(f"--- Task {task_id} Batch Delete Errors ---")
        ...
        refresh_data = {
            ...
            'repo': refresh_repo_context, 
            'path': refresh_path_context, 
            'action': 'delete' # Use 'delete' so existing refresh logic in process_queue handles it
        }
        update_queue.put((task_id, final_message, 100, True, refresh_data))
    # --- END: New Batch Delete GitHub Items Threading ---


    # Comment out or remove old single-item delete thread functions
            ...
            try:
                ...
                if item_type == 'file':
                    ...
                    if contents:
                         ...
                         else: total_items_estimated += len(contents) -1 # Only count new items beyond the current dir itself
                         ...
                         success_count += 1 # Count directory itself as processed
